...
The RNAseq bioinformatics pipeline is built using [Nextflow](https://www.nextflow.io/) and adapted from [nf-core/rnaseq pipeline](https://github.com/nf-core/rnaseq). A brief summary of pipeline:
...
6. Sample comparison
    * Sample similarity matrix and MDS plot ([`edgeR`](https://bioconductor.org/packages/release/bioc/html/edgeR.html))
    * Heatmap of expression patterns of top genes ([`DESeq2`](https://bioconductor.org/packages/release/bioc/html/DESeq2.html)) 
...
The general statistics table gives an overview of some important stats of your samples. For example, how many reads were in each run, what are the alignment rates, and how many reads were assigned to genes, etc. These stats are collected from different sections of the report to give you a snapshot.
...
The pipeline uses [`edgeR`](https://bioconductor.org/packages/release/bioc/html/edgeR.html) to generate two overview plots of all samples, when there are more than 3 samples in the study. They are generated using normalized read counts of genes.
...
## Workflow summary
This section lists some important parameters of this particular study. The often include which reference genome is used, how the trimming was done, and false discovery rate (FDR) and fold change thresholds used in differential gene expression analysis and gene set enrichment analysis. 
